/**
 * Shared record → synthesize → simulate → verify pipeline.
 *
...
export interface SimulatePipelineInput extends SynthPipelineInput {}
...
export interface VerifyPipelineResult extends SimulatePipelineResult {
  readonly ok: boolean;
  readonly failures: readonly string[];
}

/**
 * Run the dry-run suite and assert every scenario matches its expectation
 * (same contract as `npm run demo`).
 */
export function pipelineVerify(input: SimulatePipelineInput = {}): VerifyPipelineResult {
